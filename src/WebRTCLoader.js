...
const PLAYER_WEBRTC_WS_ERROR = 501;
const PLAYER_WEBRTC_ADD_ICECANDIDATE_ERROR = 502;
const PLAYER_WEBRTC_SET_REMOTE_DESC_ERROR = 503;
const PLAYER_WEBRTC_CREATE_ANSWER_ERROR = 504;
const PLAYER_WEBRTC_SET_LOCAL_DESC_ERROR = 505;
const PLAYER_WEBRTC_INTERNAL_ERROR = 506;
const PLAYER_WEBRTC_NETWORK_SLOW = 510;
const PLAYER_WEBRTC_UNEXPECTED_DISCONNECT = 511;
const ERRORS = { 
    codes: {
        501: {
            code: 501,
        ...
        502: {
            code: 502,
            message: "Add IceCandidate Error",
            error: null
        },        
        503: {
            code: 503,
            message: "Set Remote Description Error",
            error: null
        },        
        504: {
            code: 504,
            message: "Create Answer Error",
            error: null
        },        
        505: {
            code: 505,
            message: "Set Local Description Error",
            error: null
        },    
        506: {
            code: 506,
...
const WebRTCLoader = function (
    ...
    const loadCallback = callbacks.loadCallback || callbackPlaceholder;
    const connectedCallback = callbacks.connectedCallback || callbackPlaceholder;
    const internalErrorCallback = callbacks.internalErrorCallback || callbackPlaceholder;
    const peerConnectionPreparedCallback = callbacks.peerConnectionPreparedCallback || callbackPlaceholder;
    const omeP2PModeCallback = callbacks.omeP2PModeCallback || callbackPlaceholder;
    const contentLevelChangedCallback = callbacks.contentLevelChangedCallback || callbackPlaceholder;
    const resetCallback = callbacks.resetCallback || callbackPlaceholder;
    const peerConnectionDestroyedCallback = callbacks.peerConnectionDestroyedCallback || callbackPlaceholder;
    ...
    if (config &&
        config.generatePublicCandidate === false) {

        generatePublicCandidate = config.generatePublicCandidate;
    }
    ...
    function createMainPeerConnection(id, peerId, sdp, candidates, iceServers) {
        ...
        if (config && config.iceServers) {
            ...
            peerConnectionConfig.iceServers = config.iceServers;

            if (config.iceTransportPolicy) {

                peerConnectionConfig.iceTransportPolicy = config.iceTransportPolicy;
            }
        } else if (iceServers) {
        ...
        try {
            ...
            //.trigger(PEER_CONNECTION_PREPARED, peerConnection);
            if(peerConnectionPreparedCallback){
                peerConnectionPreparedCallback(peerConnection);
            }
        ...
        peerConnection.ontrack = function (e) {
            ...
            if (config && config.playoutDelayHint) {

                let hint = config.playoutDelayHint;
    ...
    function initWebSocket() {
        ...
        try {
            ...
            ws.onmessage = function (e) {
                ...
                OvenPlayerConsole.log('msg', message);
                ...
                if (message.command === 'offer') {
                    ...
                    if (message.peer_id === 0) {
                        ...
                        if(omeP2PModeCallback)
                            omeP2PModeCallback(false);
                    } else {
                        if(omeP2PModeCallback)
                            omeP2PModeCallback(true);
                    }
                }
                ...
                if (message.command === 'notification') {
                    ...
                    if (message.type === 'rendition_changed') {
                        ...
                        if(contentLevelChangedCallback)
                            contentLevelChangedCallback({
                                isAuto: autoQuality,
    ...
    function closePeer(error) {
        ...
        if (mainPeerConnectionInfo) {
            ...
            peerConnectionDestroyedCallback();
            mainPeerConnectionInfo = null;
        }
